				...
				<table class="table table-condensed">
					<thead>
						<tr class="cart_menu">
							...
							<td class="total">Porucio</td>
							<td class="total">Proizvod je poslat</td>
							<td></td>
						</tr>
					</thead>
					<tbody>
					...
					if($loggedAdmin == FALSE) die("Samo administrator moze brisati predmete");
					
						$result= queryMysql("SELECT * FROM `cart` ORDER BY id DESC "); //zadnji narucen prvi na spisku
						$num=$result->num_rows;	
					...
					?>
						...
						<tr>
							<td class="cart_product">
								<a href="proizvod_karakteristike.php?a=<?php echo $row['product_id'];?>"> <img src="<?php echo $row2['file_path']; ?>" alt=""></a>
							</td>
							...
							<td class="cart_total">
								...
								<?php 
								$result3=queryMysql("SELECT * FROM members WHERE id='".$row['user_id']."' ");
								$row3=$result3->fetch_assoc();
								?>
								 <?php  echo $row3['name']." ".$row3['last_name']; ?><br>
								<?php echo $row3['adress']; ?><br>
								 <?php echo $row3['mobile']; ?>	<br>
								komada: <?php echo $row['number_of_item']; ?>	<br>								
								 id korisnika: <?php echo $row3['id']; ?> <br>
								</p>
							</td>
							<td class="cart_delete">
							<?php
							if($row['sent']==0) 
								echo "<p class='cart_total_price'> posalji <a class='cart_quantity_delete' href='porudzbine.php?a=".$row['id']." '><i class='fa fa-times'></i></a> </p>";
							else echo"poslato";
							?>
							</td>
						</tr>
