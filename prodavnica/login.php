...
$connection2= mysqli_connect($dbhost, $dbusetr, $dbpass, $dbname,"3308");
	...
	if($user == '' || $pass =='' ){
		$error="Unesi potrebne podatke <br>";
			...
			$result=mysqli_query($connection,"select userExists('$user')");
			$res=mysqli_fetch_row($result);
			if($res[0] == false){
				$error="<span class='error'>Korisnik ne postoji</span><br><br>";  
				queryMysql("INSERT INTO `LogData` (`date`,`user`, `pass`, `success`,`ip`) VALUES (NOW(),'$user','$hpass','0','$ip')");
			}
			else {
				//$row=result->fetch() 
				...
				$res=mysqli_fetch_row($result);
				if($res[0] == false){				
					$error="<span class='error'>Neispravna lozinka</span><br><br>";
					queryMysql("INSERT INTO `LogData` (`date`,`user`, `pass`, `success`,`ip`) VALUES (NOW(),'$user','$hpass','0','$ip')");
				}
				else{
					$result=mysqli_query($connection,"select getId('$user')");
					$res=mysqli_fetch_row($result);
					$id=$res[0];
					$_SESSION['id']=$id;
					$_SESSION['user']=$user;
					queryMysql("update members set online=true where id=$id ");
					queryMysql("INSERT INTO `LogData` (`date`,`user`, `pass`, `success`,`ip`) VALUES (NOW(),'$user','$hpass','1','$ip')");
					die("Uspesno ste se ulogovali. Kliknite <a href='index.php'>ovde </a>da nastavite. <br><br>");
				}
			}
	}
